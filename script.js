...
class Products {
    async getProducts() {
        try {
            let result = await fetch("https://67509d6869dc1669ec1bc84f.mockapi.io/product");
...
class UI {
    displayProducts(products) {
        ...
        products.forEach((item) => {
            const randomImage = `${item.image}?random=${item.id}`;
            item.randomImage = randomImage;
            result += `
            ...
            if (inCart) {
                ...
                button.addEventListener("click", (event) => {
                    ...
                    let cartItem = {
                        ...
                        image: `${Storage.getProduct(id).image}?random=${id}`,
                        amount: 1,
                    };
                    ...
                    //*Sepet Tutarını Kaydet
                    this.saveCartValues(cart);
                    //* Sepet Öğesini Görüntüle
    ...
    cartLogic() {
        ...
        cartContent.addEventListener("click", (event) => {
            if (event.target.classList.contains("cart-remove-btn")) {
                ...
                let addAmount = event.target.closest("button");
    ...
    getSingleButton(id) {
        return buttonsDOW.find((button) => button.dataset.id === id);
    }
}
